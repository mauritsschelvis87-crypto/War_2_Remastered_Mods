"""Shared custom-unit preset model for War2 Voice Compare → Content Studio."""
...
PRESETS_DIR = Path(os.environ.get("LOCALAPPDATA", str(Path.home()))) / "War2VoiceCompare" / "unit-presets"
REPO_EXAMPLE_DIR = Path(__file__).resolve().parent / "unit-presets"

# baseUnitType → spells / role (pudspec Appendix A). Prefer unique hero ids for campaigns.
...
def ensure_presets_dir() -> Path:
    ...
    # Seed example presets once (do not overwrite user edits).
...
def load_presets() -> list[dict]:
    ...
    for path in sorted(PRESETS_DIR.glob("*.json")):
        ...
        data.setdefault("audioBank", "Knight")
...
def save_preset(preset: dict) -> dict:
    ...
    preset_id = str(preset.get("id") or slugify(display))
    ...
    cleaned = {
        ...
        "audioBank": str(preset.get("audioBank") or "Knight"),
        "stats": stats,

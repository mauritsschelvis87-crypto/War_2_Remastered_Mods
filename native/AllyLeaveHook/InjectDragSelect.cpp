// Injects DragSelectHook.dll into Warcraft II.exe and sets enabled state.
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    FARPROC setEnabled = RemoteGetProc(process, pid, L"DragSelectHook.dll", "DragSelect_SetEnabled");
    FARPROC isReady = RemoteGetProc(process, pid, L"DragSelectHook.dll", "DragSelect_IsReady");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"Could not resolve DragSelect_SetEnabled.\n");
        return 8;
    }
    ...
    if (isReady) {
        ...
        if (!ready) {
            CloseHandle(process);
            std::fwprintf(stderr, L"Drag-select pattern was not found in this game build.\n");
            return 9;
        }
    }
    ...
    std::fwprintf(stdout,
        L"Drag-select color hook %s. Log: %%TEMP%%\\war2_drag_select_hook.log\n",
...
int wmain(int argc, wchar_t** argv)
{
    ...
    const std::wstring dllPath = SiblingPath(L"DragSelectHook.dll");

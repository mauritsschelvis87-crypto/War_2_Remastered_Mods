// Injects AllyLeaveHook.dll into Warcraft II.exe and sets enabled state.
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    if (!ModuleLoaded(pid, L"AllyLeaveHook.dll")) {
    ...
    FARPROC setEnabled = RemoteGetProc(process, pid, L"AllyLeaveHook.dll", "AllyLeave_SetEnabled");
    FARPROC isReady = RemoteGetProc(process, pid, L"AllyLeaveHook.dll", "AllyLeave_IsReady");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"Could not resolve AllyLeave_SetEnabled.\n");
        return 8;
    }
    ...
    if (isReady) {
        ...
        if (!ready) {
            CloseHandle(process);
            std::fwprintf(stderr, L"Hook pattern was not found in this game build.\n");
            return 9;
        }
    }
...
int wmain(int argc, wchar_t** argv)
{
    ...
    const std::wstring dllPath = SiblingPath(L"AllyLeaveHook.dll");

// Injects NetworkMonitorHook.dll into Warcraft II.exe and sets enabled state.
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    if (!ModuleLoaded(pid, L"NetworkMonitorHook.dll")) {
    ...
    FARPROC setEnabled = RemoteGetProc(process, pid, L"NetworkMonitorHook.dll", "NetworkMonitor_SetEnabled");
    FARPROC isReady = RemoteGetProc(process, pid, L"NetworkMonitorHook.dll", "NetworkMonitor_IsReady");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"Could not resolve NetworkMonitor_SetEnabled.\n");
        return 8;
    }
    ...
    if (isReady) {
        ...
        if (!ready) {
            CloseHandle(process);
            std::fwprintf(stderr, L"Network monitor hook was not ready in this game build.\n");
            return 9;
        }
    }
    ...
    std::fwprintf(stdout,
        L"Network monitor %s. Log: %%TEMP%%\\war2_network_monitor.log\n",
...
int wmain(int argc, wchar_t** argv)
{
    ...
    const std::wstring dllPath = SiblingPath(L"NetworkMonitorHook.dll");

// Injects ObserveHook.dll into Warcraft II.exe and sets enabled state.
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    if (!ModuleLoaded(pid, L"ObserveHook.dll")) {
    ...
    FARPROC setEnabled = RemoteGetProc(process, pid, L"ObserveHook.dll", "Observe_SetEnabled");
    FARPROC isReady = RemoteGetProc(process, pid, L"ObserveHook.dll", "Observe_IsReady");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"Could not resolve Observe_SetEnabled.\n");
        return 8;
    }
    ...
    if (isReady) {
        ...
        if (!ready) {
            CloseHandle(process);
            std::fwprintf(stderr, L"Observe hook pattern was not found in this game build.\n");
            return 9;
        }
    }
    ...
    std::fwprintf(stdout,
        L"End-game Observe button %s. Log: %%TEMP%%\\war2_observe_hook.log\n",
...
int wmain(int argc, wchar_t** argv)
{
    ...
    const std::wstring dllPath = SiblingPath(L"ObserveHook.dll");

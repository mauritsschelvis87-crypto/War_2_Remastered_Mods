// Injects LobbyMapClickHook.dll into Warcraft II.exe and sets enabled state.
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    if (!ModuleLoaded(pid, L"LobbyMapClickHook.dll")) {
    ...
    if (!ModuleLoaded(pid, L"LobbyMapClickHook.dll")) {
        CloseHandle(process);
        std::fwprintf(stderr, L"LobbyMapClickHook.dll did not load into the game.\n");
        return 7;
    }

    FARPROC setEnabled = RemoteGetProc(process, pid, L"LobbyMapClickHook.dll", "SetEnabled");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"SetEnabled export not found.\n");
        return 8;
    }
    ...
    std::fwprintf(stdout,
        L"Lobby map click hook %s. Log: %%TEMP%%\\war2_lobby_map_click.log\n",
...
int wmain(int argc, wchar_t** argv)
{
    ...
    const std::wstring dllPath = SiblingPath(L"LobbyMapClickHook.dll");

// Injects ChatNameColorHook.dll and toggles PauseNameColor_SetEnabled
// (colors names in "X resumed the game." / pause-style system lines).
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    FARPROC setEnabled = RemoteGetProc(process, pid, L"ChatNameColorHook.dll", "PauseNameColor_SetEnabled");
    FARPROC isReady = RemoteGetProc(process, pid, L"ChatNameColorHook.dll", "ChatNameColor_IsReady");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"Could not resolve PauseNameColor_SetEnabled.\n");
        return 8;
    }
    ...
    if (isReady) {
        ...
        if (!ready) {
            CloseHandle(process);
            std::fwprintf(stderr, L"name-color pattern was not found in this game build.\n");
            return 9;
        }
    }
    ...
    if (!setThread) {
        CloseHandle(process);
        std::fwprintf(stderr, L"CreateRemoteThread(PauseNameColor_SetEnabled) failed (%lu).\n", GetLastError());
        return 10;
    }
    ...
    std::fwprintf(stdout,
        L"Pause/resume name colors %s. Log: %%TEMP%%\\war2_chat_name_color_hook.log\n",

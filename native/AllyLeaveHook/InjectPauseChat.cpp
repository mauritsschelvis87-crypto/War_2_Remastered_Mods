// Injects PauseChatHook.dll into Warcraft II.exe and sets enabled state.
...
int InjectAndSet(const std::wstring& dllPath, bool enable)
{
    ...
    if (!ModuleLoaded(pid, L"PauseChatHook.dll")) {
    ...
    FARPROC setEnabled = RemoteGetProc(process, pid, L"PauseChatHook.dll", "PauseChat_SetEnabled");
    FARPROC isReady = RemoteGetProc(process, pid, L"PauseChatHook.dll", "PauseChat_IsReady");
    if (!setEnabled) {
        CloseHandle(process);
        std::fwprintf(stderr, L"Could not resolve PauseChat_SetEnabled.\n");
        return 8;
    }
    ...
    if (isReady) {
        ...
        if (!ready) {
            CloseHandle(process);
            std::fwprintf(stderr, L"Pause-chat pattern was not found in this game build.\n");
            return 9;
        }
    }
    ...
    std::fwprintf(stdout,
        L"Chat during pause %s. Log: %%TEMP%%\\war2_pause_chat_hook.log\n",
...
int wmain(int argc, wchar_t** argv)
{
    ...
    const std::wstring dllPath = SiblingPath(L"PauseChatHook.dll");
